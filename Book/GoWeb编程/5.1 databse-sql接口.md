...
## sql.Register
注册数据库驱动，在init函数中调用 `Register(name string, driver driver.Driver)` 完成本驱动的注册
...
>      )
>      
> `_` 用来忽略变量赋值的占位符, 这儿使用 `_` 的意思是引入后面的包名而不直接使用这个包中定义的函数，变量等资源。
> 
> 引入数据库驱动包之后会自动去调用init函数，然后在init函数里面注册这个数据库驱动，这样我们就可以在接下来的代码中直接使用这个数据库驱动了。
